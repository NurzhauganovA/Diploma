...
class GetLastRegisteredUsersService:
    ...
    def set_response_data(cls, response_data):
        ...
        for user in response_data:
            data.append({"photo_avatar": user.get_photo()})
...
class DailyUsersService:
    ...
    def get_daily_users(self) -> WeekLoginCount:
        ...
        for day in range(6, -1, -1):
            ...
            result[self.week[date.isoweekday()]] = User.objects.filter(
                login_days__icontains=date.strftime("%d.%m.%Y")
            ).count()
...
class SchoolScheduleService:
    ...
    def week_day_breaks(schedule):
        ...
        for i in range(len(schedule) - 1):
            ...
            break_duration = (
                next_section.datetime
                - (
                    current_section.datetime
                    + datetime.timedelta(hours=current_section.duration)
                )
            ).total_seconds() // 60
    ...
    def get_school_schedule(self):
        ...
        this_week_sections = SubjectSection.objects.filter(
        ...
        wednesday_schedule = self.week_day_schedule(
            3, this_week_sections, start_of_week
        )
...
class SchoolSectionAttendanceService:
    ...
    def get_section_attendance(self):
        ...
        student_attendance = SectionAction.objects.filter(
            student=Student.objects.get(user=self.user)
        )

        for status in SectionActionStatus.get_choices():
            attendance_dict[status[1]] = student_attendance.filter(
                status=status[0]
            ).count()
        ...
        attended_status_count = SectionAction.objects.filter(
            student=Student.objects.get(user=self.user),
            status=SectionActionStatus.ATTENDED,
        ).count()
        attended_status_percent = int(
            (attended_status_count / len(student_attendance)) * 100
        )
