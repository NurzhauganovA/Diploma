...
from dashboard.services import (
    GetSchoolRegionsService,
    GetLastRegisteredUsersService,
    GetCountNewUsersThisMonthService,
...
def home(request):
    ...
    last_registered_users = (
        GetLastRegisteredUsersService().get_last_registered_users()
    )  # work
    count_last_registered_users = (
        GetCountNewUsersThisMonthService().get_count_new_users_current_month()
    )  # work
    overall_users_analytics = (
        GetOverallGoalUsersService().get_overall_users_analytics()
    )  # work
...
def student_dashboard(request):
    ...
    attendance_context = SchoolSectionAttendanceService(
        request.user
    ).get_section_attendance()

    total_homeworks = SectionHomework.objects.filter(
        section__subject__classroom=Student.objects.get(user=request.user).stud_class
    ).count()
    done_total_homeworks = SectionHomeworkAnswer.objects.filter(
    ...
    homeworks = {
        ...
        "percent": int(done_total_homeworks / total_homeworks * 100)
        if total_homeworks != 0
        else 0,
    }

    total_tests = SectionTests.objects.filter(
        section__subject__classroom=Student.objects.get(user=request.user).stud_class
    ).count()
    done_total_tests = SectionTestsAnswer.objects.filter(
        student=Student.objects.get(user=request.user)
    ).count()
    ...
    context = {
        ...
        "homeworks": homeworks,
        "tests": tests,
        "total_subjects": SubjectSection.objects.filter(
            subject__classroom=Student.objects.get(user=request.user).stud_class
        ).count(),
    }
