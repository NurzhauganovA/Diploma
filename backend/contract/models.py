...
class Discount(models.Model):
    ...
    school = models.ForeignKey(
        School, on_delete=models.SET_NULL, null=True, db_column="school"
    )
...
class Contract(models.Model):
    ...
    payment_type = models.CharField(
        max_length=255,
        choices=ContractPaymentPeriodType.choices,
        default=ContractPaymentPeriodType.MONTHLY,
    )
    status = models.CharField(
        max_length=255, choices=ContractStatus.choices, default=ContractStatus.FORMED
    )
    final_amount = models.DecimalField(max_digits=10, decimal_places=2)
    edu_year = models.CharField(max_length=255)
    classroom = models.ForeignKey(
        Class, on_delete=models.SET_NULL, null=True, db_column="classroom"
    )
    discount = models.ManyToManyField(
        Discount, db_column="discount", related_name="discount"
    )
...
class ContractMonthPay(models.Model):
    ...
    contract = models.ForeignKey(
        Contract, on_delete=models.SET_NULL, null=True, db_column="contract"
    )
...
class ContractDayPay(models.Model):
    ...
    contract_month = models.ForeignKey(
        ContractMonthPay,
        on_delete=models.SET_NULL,
        null=True,
        db_column="contract_month",
    )
