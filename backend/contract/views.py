...
def get_all_contracts(req: HttpRequest):
    school = School.objects.get(id=GetSchoolPartData(req.user.id).get_school_pk())
...
def get_contract_info(request: HttpRequest, pk: int) -> JsonResponse:
    if request.method == "GET":
        ...
        contract_info = {
            "id": contract.id,
            "name": contract.name,
            "student_full_name": contract.student.user.full_name
            if contract.student.user
            else "No data",
            "date": contract.date.strftime("%d.%m.%Y") if contract.date else "No data",
            "final_amount": int(contract.final_amount)
...
def change_contract_data(request: HttpRequest, contract_number: str):
    if request.method == "PUT":
        try:
            ...
            contract.date = (
                datetime.strptime(data.get("agreementDate"), "%d.%m.%Y").strftime(
                    "%Y-%m-%d"
                )
                if data.get("agreementDate")
                else contract.date
            )
            contract.final_amount = (
                data.get("contractSum")
                if data.get("contractSum")
                else contract.final_amount
            )
            contract.status = (
                data.get("contractStatus")
                if data.get("contractStatus")
                else contract.status
            )
            contract.payment_type = (
                data.get("contractPaymentPeriod")
                if data.get("contractPaymentPeriod")
                else contract.payment_type
            )
            contract.save()
