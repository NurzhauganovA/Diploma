...
def register(request: HttpRequest):
    if request.method == "POST":
        ...
        new_user = User.objects.create(
            full_name=full_name, mobile_phone=mobile_phone, role=role, password=password
        )
...
def verify_email(request: HttpRequest):
    ...
    if request.method == "POST":
        ...
        if verify_account(email=email, code=int(code)):
            ...
            if not user.check_password(password):
                return JsonResponse(
                    {"message": "Password is incorrect!", "status": 400}
                )
        ...
        return JsonResponse(
            {"message": "Verification code is incorrect!", "status": 400}
        )
        ...
        if process_this_frame:
            ...
            for face_encoding in face_encodings:
                matches = face_rec.compare_faces(
                    know_face_encodings, face_encoding, tolerance=0.39
                )
                name = "Unkown"
